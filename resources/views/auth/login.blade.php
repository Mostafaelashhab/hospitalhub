        ...
        <div class="w-full lg:w-1/2 flex flex-col">
            ...
            <div class="flex-1 flex items-center justify-center px-6 py-8 lg:px-12">
                <div class="w-full max-w-md">
                    ...
                    {{-- Phone Login --}}
                    <div x-data="whatsappCheck()" x-init="check()">
                        <a :href="online ? '{{ route('otp.login') }}' : '#'"
                           :class="online ? 'bg-emerald-600/10 text-emerald-400 hover:bg-emerald-600/20 border-emerald-600/30 cursor-pointer' : 'bg-gray-800/50 text-gray-500 border-gray-700 cursor-not-allowed'"
                           class="w-full py-3.5 font-semibold rounded-xl transition-all border flex items-center justify-center gap-2 mb-3"
                           @click="if(!online) $event.preventDefault()">
                            {{-- Loading spinner --}}
                            <svg x-show="checking" class="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                            {{-- WhatsApp icon --}}
                            ...
                            <span x-show="checking">@if(app()->getLocale()==='ar') جاري التحقق... @else Checking... @endif</span>
                            <span x-show="!checking && online">@if(app()->getLocale()==='ar') الدخول بالواتساب @else Login with WhatsApp @endif</span>
                            <span x-show="!checking && !online">@if(app()->getLocale()==='ar') الواتساب غير متاح حالياً @else WhatsApp Unavailable @endif</span>
                        </a>
                    </div>

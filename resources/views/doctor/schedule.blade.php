<x-dashboard-layout>
    <x-slot name="title">{{ __('app.schedule') }}</x-slot>
    <x-slot name="pageTitle">{{ __('app.schedule') }}</x-slot>

    <x-slot name="sidebar">
        @include('partials.doctor-sidebar', ['active' => 'schedule'])
    </x-slot>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">{{ __('app.schedule') }}</h2>
    ...
    function scheduleManager() {
        ...
        return {
            shifts,
            timeOptions,
            getShifts(day) { return this.shifts.filter(s => s.day === day); },
            countShifts(day) { return this.shifts.filter(s => s.day === day).length; },
            dayHasSchedule(day) { return this.shifts.some(s => s.day === day); },
            addShift(day) {
                this.shifts.push({ id: nextId++, idx: this.shifts.length, day, start_time: '09:00', end_time: '17:00', slot_duration: '30' });
                this.reindex();
            },
            removeShift(id) { this.shifts = this.shifts.filter(s => s.id !== id); this.reindex(); },
            reindex() { this.shifts.forEach((s, i) => s.idx = i); }
        };
    }
    </script>
</x-dashboard-layout>

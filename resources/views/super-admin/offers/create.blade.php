<x-dashboard-layout>
    <x-slot name="title">{{ __('app.create_offer') }}</x-slot>
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('app.create_offer') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('app.create_offer_desc') }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6" x-data="{ type: '{{ old('type', 'discount') }}' }">
        <form method="POST" action="{{ route('super.offers.store') }}" enctype="multipart/form-data">
            @csrf

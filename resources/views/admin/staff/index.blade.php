<x-dashboard-layout>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">{{ __('app.staff') }}</h2>
            <p class="text-sm text-gray-500 mt-1">{{ __('app.manage_staff_desc') }}</p>
        </div>
        @if(auth()->user()->hasPermission('staff.create'))
        <a href="{{ route('dashboard.staff.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl shadow-sm shadow-indigo-500/20 transition-all">

            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="appointmentForm()">
                ...
                <div>
                    ...
                    <select name="doctor_id" required x-model="doctorId" @change="fetchServices(); fetchSlots()"
                        ...
                        <option value="{{ $doctor->id }}" {{ old('doctor_id') == $doctor->id ? 'selected' : '' }}>{{ $doctor->name }} — {{ app()->getLocale() === 'ar' ? $doctor->specialty?->name_ar : $doctor->specialty?->name_en }} ({{ number_format($doctor->consultation_fee) }} {{ __('app.currency') }})</option>
                        @endforeach
                    </select>
                </div>

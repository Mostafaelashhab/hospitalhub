<x-dashboard-layout>
    <x-slot name="title">{{ __('app.edit_expense') }}</x-slot>
    <x-slot name="sidebar">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-xl sm:text-2xl font-bold text-gray-900">{{ __('app.edit_expense') }}</h2>
        </div>
    </div>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('dashboard.expenses.update', $expense) }}" enctype="multipart/form-data" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="amount" step="0.01" min="0.01" value="{{ old('amount', $expense->amount) }}" required class="w-full border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500">
                    @error('amount') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('app.date') }} <span class="text-red-500">*</span></label>
                    <input type="date" name="expense_date" value="{{ old('expense_date', $expense->expense_date->format('Y-m-d')) }}" required class="w-full border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2" x-data="{ method: '{{ old('payment_method', $expense->payment_method) }}' }">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $expense->description) }}</textarea>
            ...
            <div>
                ...
                <div class="mb-2">
                    <img src="{{ Storage::url($expense->receipt_path) }}" alt="Receipt" class="w-32 h-32 object-cover rounded-xl border border-gray-200">
                </div>
                @endif
